...
/**
 * each cluster contain a group of vector 
 * each vector is a group point in same cluster
 * data in vector is a point coordinate
*/
...
public class AlgoDBSCAN {
	 ...
	 */
	public List<Cluster> runAlgorithm(double[][] inputeData,int pointNumber, int minPts, double epsilon) throws NumberFormatException, IOException {
		...
		List<DoubleArray> points = new ArrayList<DoubleArray>();
	        ...
	        List<DoubleArray> resultVector=new ArrayList<>();           
			  //add input data as point to cluster
		          for (int i = 0; i <pointNumber; i++) {
                             // add the vector to the list of vectors
                        points.add(new DoubleArrayDBS(inputeData[i], String.valueOf(i))); 
...
//		System.out.println(kdtree.toString());
		...
		kdtree = null;
             ...
             /*  for(Cluster cl : clusters){
                    for(int i=0;i<cl.getVectors().size();i++){
                        for(int j=0;j<cl.getVectors().get(i).data.length;j++)
                            System.out.print(cl.getVectors().get(i).data[j]+" ");
                       System.out.print("- ");
                    }
                    System.out.println();
                }
                */
              /*   for(Cluster cl : clusters){
                       double[] center=new double[cl.getVectors().get(0).data.length];
                            for(int i=0;i<cl.getVectors().size();i++){
                        for(int j=0;j<cl.getVectors().get(i).data.length;j++){
                            center[j]+=cl.getVectors().get(i).data[j];
                        }
                           
                      
                    }
                     for(int j=0;j<center.length;j++){
                            center[j]/=cl.getVectors().size();
                        }
                     resultVector.add(new DoubleArray(center));
